...
Data::SwitchPermission { name, enabled } => {
    #[cfg(windows)]
    {
        ...
        let _ = OpenOptions::new()
            .create(true)
            .append(true)
            .open(r"C:\temp\cm_debug.txt")
            .and_then(|mut f| writeln!(f, "CM received SwitchPermission: {}={}", name, enabled));
    }
    // TODO: 这里需要调用 CM 实际的权限处理逻辑。
    // 临时方案：直接调用 platform::block_input（但会屏蔽远程输入，仅用于测试）
    // 如果你能接受暂时屏蔽远程输入用于验证，可以取消注释下面一行：
     let _ = crate::platform::block_input(enabled);
}
